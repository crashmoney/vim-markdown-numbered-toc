...
function! s:Toc(...)
    ...
    setlocal modifiable
    let l:numheader = ['', '', '', '', '', '', ''] " *ADDED*
    let l:prevlevel = 0 " *ADDED*
    let l:level = 0
    for i in range(1, line('$'))
        ...
        " atx headers
        if match(d.text, "^#") > -1
            let l:prevlevel = l:level " *ADDED*
            let l:level = len(matchstr(d.text, '#*', 'g'))-1
            let d.text = substitute(d.text, '\v^#*[ ]*', '', '')
            let d.text = substitute(d.text, '\v[ ]*#*$', '', '')
            " fill the numheader list " *ADDED*
            if l:prevlevel < l:level " *ADDED*
                let l:numheader[l:level] = 1 " *ADDED*
            elseif l:prevlevel > l:level " *ADDED*
                let l:numheader[l:level] += 1 " *ADDED*
                " clean numheader
                for j in [1,2,3,4,5,6] " *ADDED*
                    if j > l:level " *ADDED*
                        let l:numheader[j] = '' " *ADDED*
                    endif " *ADDED*
                endfor " *ADDED*
            else " *ADDED*
                let l:numheader[l:level] += 1 " *ADDED*
        ...
        let l:concatnumheader = join(l:numheader, '.') " *ADDED*
        let l:concatnumheader = substitute(l:concatnumheader, '\.\+$', '', '') " *ADDED*
        call setline(i, repeat('  ', l:level). l:concatnumheader.'  '. d.text)
    endfor
    setlocal nomodified
